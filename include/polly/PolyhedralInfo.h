...
/// Set type for Dependence Instruction Pairs.
using DependencePairsSetTy =
    llvm::SmallSet<std::pair<llvm::Instruction *, llvm::Instruction *>, 4>;
...
public:
  ...
  /// Check if write conflicts exists in the Loop
  bool isWriteConflict(llvm::Loop *L) const;

  /// Check if a given loop is parallel.
  ///
  /// @param L The loop.
  /// @param DependencePairsSetTy Return Set of dependent instruction pairs.
  ///
  /// @return  Returns true, if loop is parallel false otherwise.
  bool isParallel(llvm::Loop *L, DependencePairsSetTy &) const;
...
private:
  ...
  bool checkParallel(llvm::Loop *L,
                     __isl_give isl_pw_aff **MinDepDistPtr = nullptr,
                     DependencePairsSetTy *ConflictingPairs = nullptr) const;
  __isl_give isl_union_map *
  addKnownParamValues(__isl_give isl_union_map *Schedule,
                      llvm::DenseMap<unsigned, int> ParamVals) const;
